...
**重要**：在生成 PRD 文档前，必须先获取当前日期用于文件命名。
...
将获取到的日期保存为变量 `CURRENT_DATE`，后续文件保存时将使用此日期。
...
- **日期获取**：必须使用系统命令 `date +%Y-%m-%d` 获取当前日期，**不要**手动猜测或使用 LLM 记忆中的日期
- **文件路径构建**：使用第七步获取的 `CURRENT_DATE` 变量替换 `{YYYY-MM-DD}` 占位符
